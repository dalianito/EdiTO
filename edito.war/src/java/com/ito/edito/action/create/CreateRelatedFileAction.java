...
public class CreateRelatedFileAction extends AjaxAction {
		...
		if (file != null) {
			...
			for (int i = 0; i < fileNum; i++) {
				...
				try {
					...
					relatedFile.setTypeUuid(typeUuid, false);
					relatedFile.setName(realName);
					relatedFile.setPath(saveName);
					relatedFile.setUploadPersonUuid(USER.getPersonUuid(), false);
					relatedFile.setUploadTime(new Date());

					userService.insertRelatedFile(relatedFile, USER.getAccount());
