...
function execute(){
	...
	var pageMark = "record";
	var Uuid = $("#Uuid").val();
	$.post(cp+"/ajaxs/wechat/common/work/wechat_query_workRecord.whtml", {nowTime:nowTime,pageMark:pageMark,Uuid:Uuid}, function(data){
		...
		for(var i=0;i<data[0].length;i++){
			...
			buffer += "<td align='center'>"+data[0][i][3]+"</td>";
			buffer += "<td align='center'>"+data[0][i][4]+"</td>";
			buffer += "<td align='center'>"+data[0][i][5]+"</td>";
			if(data[0][i][4] == "已审阅"){
				buffer += "<td align='center'><a href='javascript:editInstance(\""+data[0][i][0]+"\",\"/views/wechat/common/work/forward_view_workRecord.whtml\")'><img src="+pic+"></a>";
			}else{
				buffer += "<td align='center'><a href='javascript:editInstance(\""+data[0][i][0]+"\",\"/whttps/wechat/common/work/edit_wechat_work_record.whtml\")'><img src="+pic+"></a>";
			}
			buffer += "</td>";
			buffer += "</tr>";
		}
		...
		if(data[1].length == 0){
			var buff = "今天还没写日志，<br>去写<a href=\""+cp+"/whttps/wechat/common/work/create_wechat_work_record.whtml_"+Uuid+"\">工作日志</a>";
			$("#ifresult").html(buff);
		}
		else if(data[1] == "已审阅"){
			var buff = "今天的工作日志已被审阅";
			$("#ifresult").html(buff);
		}
		else {
			var buff = "今天已经写了日志，去<a href='javascript:editInstance(\""+data[1]+"\",\"/whttps/wechat/common/work/edit_wechat_work_record.whtml\")'>编辑</a>";
			$("#ifresult").html(buff);
		}
	}, "json");
}
