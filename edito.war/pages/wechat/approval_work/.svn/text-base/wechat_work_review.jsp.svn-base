...
function execute(){
	...
	$.post(cp+"/ajaxs/wechat/approval/work/query_workRecord_review.whtml", {nowTime:nowTime,Uuid:Uuid}, function(data){
		...
		buffer += "<tr><th width='35%'>记录人</th><th width='35%'>组织机构</th><th width='30%'>审阅</th></tr>";
		for(var i=0;i<data.length;i++){
			if(data[i][0] != null){
				if(data[i][4] == "已审阅"){
					...
					buffer += "<td align='center'><a href='javascript:editInstance(\""+data[i][0]+"\",\"/whttps/wechat/approval/work/forward_workRecord_review.whtml\")'><img src="+pic+"></a>";
					buffer += "</td>";
					buffer += "</tr>";
				}
			}
		...
		var content = "未审阅<font color='red'>"+notreviewCount+"</font>人，已审阅<font color='red'>"+reviewCount+"</font>人 <br> 查看已审阅的<a href='#' onclick='goreviewQuery()'>工作日志</a>"; 
...
function goreviewQuery(){
	var uuid = $("#Uuid").val();
	location.href = cp + "/whttps/wechat/approval/work/wechat_query_review.whtml_" + uuid;
}
</script>
