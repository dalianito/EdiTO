...
function addData(dataStore){
	...
	buffer += "<tr><th width='20'></th><th width='100'>申请人</th><th width='100'>申请时间</th><th width='80'>需求部门</th><th width='100'>企划部负责人</th><th width='100'>审批时间</th><th width='100'>主管校领导</th><th width='100'>审批时间</th><th width='140'>活动简述</th><th width='80'>审批状态</th><th width='60'>操作</th></tr>";
	if(dataStore.length > 0){
		for(var i=0; i<dataStore.length; i++){
			...
			buffer += "<td style='text-align:left'>&nbsp;"+dataStore[i][8]+"</td>";
			buffer += "<td>"+dataStore[i][9]+"</td>";
			buffer += "<td align='center'><a href='javascript:showInstance(\""+dataStore[i][10]+"\",\"\",\"/views/resource/campus/manage/campus/publicity/publicity/apply/forward_view_apply.html\",680,640)'>查看</a></td>";
			buffer += "</tr>";
		}
	}
	else {
...
<br>
<div style="font-size:16px;font-weight:bold;margin-top:20px;margin-bottom:10px">市场活动申请</div>
...
排序：<select id="sortField" onchange="execute()">
		...
		<option value="R.departmentUuid">需求部门</option>
	</select>
</div>
<br><br>
