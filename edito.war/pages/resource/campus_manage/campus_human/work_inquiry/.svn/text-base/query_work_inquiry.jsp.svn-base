...
function showContent(time, campusUuid, id, num) {
	$.post(cp+"/ajaxs/resource/campus/manage/campus/human/work/inquiry/query_all_work_inquiry.html", {data:time, campusUuid:campusUuid}, function(data){
		var buffer = "";
		...
		buffer += "<th align='center' width='110px'>日志总数</th>";
		tempTab[num][0][data[1][0].length - 1] = "日志总数";
		buffer += "<th align='center' width='110px'>应写日志总数 </th>";
			tempTab[num][0][data[1][0].length] = "应写日志总数 ";
		buffer += "</tr>";
		for(var j=0; j < data[0].length; j++) {
			buffer += "<tr>";
			for(var k = 0; k < data[1][j].length; k++) {
				if((data[1][j][k] == "1") && (k < data[1][j].length - 2)){
					buffer += "<td align='center' width='110px' onclick='javascript:showInstance(\""+data[2][j][k]+"\",\"\",\"/views/resource/campus/manage/my/campus/work/record/forward_view_record.html\",600,700)' style='cursor:hand' title='查看详细信息'>"+data[1][j][k]+"</td>";
				}else {
					buffer +="<td align='center' width='110px'>"+ data[1][j][k]+"</td>";
				}
				if(data[1][j][k].length > 1) {
...
<div style="font-family:黑体;font-size:20px;">&nbsp;工 作 日 志 总 览</div>
<br><br><br>
