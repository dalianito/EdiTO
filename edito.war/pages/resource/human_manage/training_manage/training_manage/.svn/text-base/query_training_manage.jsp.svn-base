...
function execute(){
	...
	$.ajax({
		type: "POST",
		url: cp+"/ajaxs/resource/human/manage/training/manage/training/manage/query_training_manage.html",
		data:data,
		cache: false,
		dataType: "json",
		success: function(msg){
			if(msg!=null){
				$("#result").pagination({
					totalRecord:msg[1],
					dataStore:msg[0],
					proxyUrl:cp+"/ajaxs/resource/human/manage/training/manage/training/manage/query_training_manage.html",
					data:data,
					currentPage:1,
					perPage:24,
...
function addData(dataStore){
	...
	for(var i=0;i<dataStore.length;i++){
		...
		buffer += "<a href='javascript:editInstance(\""+dataStore[i][5]+"\",\"/https/resource/human/manage/training/manage/training/manage/forward_edit_training_employee.html\")'>通过申请</a>&nbsp;";
		buffer += "</td>";
		buffer += "</tr>";
	}
	buffer += "</table></div>";
